...
import { updateSavingGoal } from '../../../lib/supabase/savingGoals';
...
export default function EditSavingGoal() {
  ...
  const parsed = goal ? JSON.parse(goal) : null;
  ...
  useEffect(() => {
    if (parsed) {
      setForm({
        ...
        goalAmount: String(parsed?.params?.goalAmount ?? ''),
        rangeType: parsed?.params?.rangeType || 'month',
        range: String(parsed?.params?.range ?? ''),
        annualInterestRate: String(parsed?.params?.annualInterestRate ?? ''),
        initialAmount: String(parsed?.params?.initialAmount ?? ''),
      });
    }
    setLoading(false);
  }, [parsed]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      await updateSavingGoal(payload);
      Alert.alert(t('common.success'), t('savingGoal.alerts.updateSuccess'));
      router.back();
    } catch (e) {
      console.error(e);
      Alert.alert(t('common.error'), t('savingGoal.alerts.updateError'));
    }
  };
  ...
  if (loading) {
    return (
      <View style={[styles.container, { justifyContent: 'center', alignItems: 'center' }]}>
        <Text style={{ color: theme.text }}>{t('savingGoal.loading')}</Text>
      </View>
    );
  }
...
EditSavingGoal.options = { headerShown: true };

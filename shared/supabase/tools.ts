...
interface BudgetPlanGroupDetails  {
    id : string
    name : string
    real_amount : number
...
export async function getUserBudgetPlans():Promise<BudgetPlanWithDetails[]>{
    ...
    if (error) {
        ...
        const promises = (data as BudgetPlan[]).map(budget => 
            getBudgetPlanDetails(budget.id, new Date().toISOString().split("T")[0])
        );
        return await Promise.all(promises);
    }
}

export async function getBudgetPlanDetails(id:string, reference_date:string):Promise<BudgetPlanWithDetails>{

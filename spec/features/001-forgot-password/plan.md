...
- **Auth → Email Templates → Reset Password**: replace the default template with the HTML in `spec/features/001-forgot-password/email-template.html`. The template uses `{{ .ConfirmationURL }}` in `href` attributes (always an `https://` URL that embeds the `redirect_to` query param) to avoid Go `html/template`'s `#ZgotmplZ` sanitization of custom URL schemes (`rumbo://`). The plain-text fallback displays `{{ .RedirectTo }}` for readability.

### Email templates

- `spec/features/001-forgot-password/email-template.html` — brand-aligned Rumbo HTML email. Spanish copy matching mobile strings (`forgot-password.*` keys in `apps/mobile/assets/locales/{en,es}.json`). CTA button and fallback link use `{{ .ConfirmationURL }}` in `href` (safe `https://` URL) and `{{ .RedirectTo }}` as display text. Paste into Supabase dashboard → Auth → Email Templates → Reset Password.
...
- **Recovery URL handling on web:** three-path approach on `/reset-password`:
  1. PKCE `?code=` → `exchangeCodeForSession(code)` (primary path).
  2. Implicit hash fragment `#access_token=...&refresh_token=...&type=recovery` → `setSession({ access_token, refresh_token })` (classic fallback).
  3. Async `onAuthStateChange` → `PASSWORD_RECOVERY` listener (mirrors mobile `apps/mobile/app/_layout.js:46`).
  4. `getSession()` check as last resort.
  
  Rationale: keeps `detectSessionInUrl: false` unchanged on the shared client, and handles whichever recovery format Supabase GoTrue emits (PKCE vs implicit) plus the real-time auth state event.
...
- **Wrong redirection / `rumbo://auth/callback` fallback:** the Supabase project's Site URL (Authentication → URL Configuration → Site URL) is set to `rumbo://auth/callback`. When GoTrue rejects a `redirectTo` URL (e.g., because it's non-HTTPS or not in the Redirect URLs allow-list), it falls back to the Site URL as the redirect destination. This causes all email links to redirect to the mobile app scheme instead of the web app.
  - **Fix:** ensure the Site URL points to the production web app (`https://rumbo-ten.vercel.app`) so web fallback works. Also ensure `http://localhost:3000/reset-password` and `rumbo://update-password` are both in the Redirect URLs allow-list (GoTrue won't fall back if the provided URL passes allow-list validation).
  - **Note:** GoTrue may reject `http://` (non-HTTPS) URLs even for localhost in some cloud configurations. For local dev, use a tunnel service (`ngrok`, `localtunnel`) that provides an HTTPS URL, or test against the production URL directly.
  - The `/reset-password` page now handles three session recovery paths: PKCE `?code=`, implicit hash fragment (`#access_token=...&type=recovery`), and the async `PASSWORD_RECOVERY` auth state event.
...
- **Mobile regression:** the email template change is shared with mobile. The `{{ .ConfirmationURL }}` substitution and copy must be validated on both platforms in the final cross-platform test task.
